...
export function initDatabase(database, config) {
...
export function insertCard(database, deck, card) {
    ...
    database
        .prepare(SQL_NOTE)
        .run(noteId, `${cardId}`, modelId, createTime, fieldsContent, sortField, parseInt(sha1(sortField).substr(0, 8), 16));
}
